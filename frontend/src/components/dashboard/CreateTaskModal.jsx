...
const CreateTaskModal = ({ isOpen, onClose, onTaskCreated, activeSpace }) => {
    ...
    if (isOpen && activeWorkspace) {
      axios.get(`/workspaces/${activeWorkspace._id}/hierarchy`)
        .then(res => {
          ...
          setLists(flatLists);
          if (activeSpace && activeSpace.type === 'list') {
            setListId(activeSpace.data._id);
          } else if (flatLists.length > 0) {
            setListId(flatLists[0]._id);
          }
        })
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!listId) {
      alert("Vui lòng chọn Danh sách trước khi tạo Task!");
      return;
    }
    setLoading(true);
    try {
      ...
      <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px', boxSizing: 'border-box' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 20, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.95 }} transition={{ type: 'spring', stiffness: 500, damping: 30 }} style={{ position: 'relative', width: '100%', maxWidth: '480px', background: 'white', borderRadius: '12px', boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 10px 10px -5px rgba(0, 0, 0, 0.04)', display: 'flex', flexDirection: 'column', overflow: 'hidden', fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
          <div style={{ padding: '20px 24px', borderBottom: '1px solid #f1f5f9', background: '#f8fafc', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <h2 style={{ margin: 0, fontSize: '18px', fontWeight: 700, color: '#0f172a' }}>Tạo công việc mới</h2>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column' }}>
            <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
              <div>
                ...
                <input autoFocus required placeholder="Ví dụ: Thiết kế giao diện trang chủ..." value={formData.title} onChange={e => setFormData({ ...formData, title: e.target.value })} onFocus={() => setFocusInput(true)} onBlur={() => setFocusInput(false)} style={{ width: '100%', padding: '12px 14px', fontSize: '14px', color: '#0f172a', background: 'white', border: focusInput ? '2px solid #6366f1' : '1px solid #cbd5e1', borderRadius: '8px', outline: 'none', boxSizing: 'border-box', transition: 'all 0.2s', boxShadow: focusInput ? '0 0 0 3px rgba(99, 102, 241, 0.1)' : '0 1px 2px 0 rgba(0, 0, 0, 0.05)' }} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '13px', fontWeight: 600, color: '#334155', marginBottom: '8px' }}>Chọn Danh sách (Bắt buộc) <span style={{ color: '#ef4444' }}>*</span></label>
            ...
            <div style={{ padding: '16px 24px', background: '#f8fafc', borderTop: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '12px' }}>
              ...
              <button type="submit" disabled={loading} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 20px', fontSize: '14px', fontWeight: 600, color: 'white', background: loading ? '#818cf8' : '#4f46e5', border: 'none', borderRadius: '8px', cursor: loading ? 'wait' : 'pointer', transition: 'all 0.2s', boxShadow: '0 1px 2px 0 rgba(0, 0, 0, 0.05)' }} onMouseEnter={e => { if(!loading) e.currentTarget.style.background = '#4338ca'; }} onMouseLeave={e => { if(!loading) e.currentTarget.style.background = '#4f46e5'; }}>{loading ? 'Đang tạo...' : 'Tạo Task'}</button>
            </div>
          </form>
        </motion.div>
      </div>
